...
if (isset($_POST['CloseAc'])) {
    $CloaseAc = $_POST['CloseAc'];
    $query = "SELECT * FROM accounts WHERE Balance= 0.0 and SavingBalance= 0.0 and AccountNo='$CloaseAc'";
    $result = mysqli_query($conn, $query) or die("query fail");
    if (mysqli_num_rows($result) > 0) {
        $Dquery = "UPDATE login SET Status = 'Closed' WHERE AccountNo = '$CloaseAc'";
