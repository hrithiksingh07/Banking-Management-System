...
if (isset($_POST['DebitCardCheck'])) {
    ...
    $idate = date("Y-m-d");
    $edate = date('Y-m-d', strtotime(' + 5 years'));
    $query = "UPDATE cards SET Status = 'Active',Verified = 'Yes', IssuedDate = '$idate', ExpiryDate = '$edate' WHERE AccountNo = '$AccountNo'";
    mysqli_query($conn, $query) or die(mysqli_error($conn));
    
    echo "Success";
}

// Verify/ Reject Debit Cards Code
...
if (isset($_POST['TSenderAc']) && isset($_POST['TReceiverAc']) && isset($_POST['MainAmount'])) {
        ...
        if ($SenderAc == $ReceiverAc) {
            ...
            if (mysqli_num_rows($SenderResult) > 0) {
                ...
                if (mysqli_num_rows($ReceiverResult) > 0) {
                    ...
                    if ($SBalance != "0") {
                        if ($SBalance > $Amount || $SBalance == $Amount) {
